...
t_listnode	*get_element(t_list *p_list, int position)
{
	t_listnode	*temp_node;
	int			i;

	i = 0;
	if (position >= p_list->current_element_count)
		return (0);
...
void	cleart_list(t_list *p_list)
{
	...
	while (i < p_list->current_element_count)
	{
	...
	p_list->current_element_count = 0;
	return ;
}
...
{
	return (p_list->current_element_count);
}
