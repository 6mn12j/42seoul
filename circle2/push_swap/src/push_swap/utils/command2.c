...
/*   Updated: 2022/03/12 21:48:23 by minjupar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	s_ab(t_all *all, t_list *t_list, char target)
{
	...
	if (target == 'a')
		add_command(all->list_command, command_count, "sa");
	else if (target == 'b')
		add_command(all->list_command, command_count, "sb");
	return ;
}
...
void	ss(t_all *all, t_list *a_list, t_list *b_list, char target)
{
	...
	command_count = all->list_command->current_node_count;
	s_ab(all, a_list, target);
	s_ab(all, b_list, target);
	if (target == 's')
		add_command(all->list_command, command_count, "ss");
	return ;
}

void	p_ab(t_all *all, t_list *start_list, t_list *target_list,
		char target)
{
	int			command_count;
	...
	if (target == 'a')
		add_command(all->list_command, command_count, "pa");
	else if (target == 'b')
		add_command(all->list_command, command_count, "pb");
	return ;
}
