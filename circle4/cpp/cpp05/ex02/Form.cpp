...
Form::Form(void)
    : type("Form"), name("undefined"), requiredExecuteGrade(150),
      requiredSignedGrade(150), isSigned(false) {}

Form::Form(std::string name)
    : type("Form"), name(name), requiredExecuteGrade(150),
      requiredSignedGrade(150), isSigned(false) {}

Form::Form(std::string type, std::string name, int requiredExecuteGrade,
           int requiredSignedGrade)
    : type(type), name(name), requiredExecuteGrade(requiredExecuteGrade),
      requiredSignedGrade(requiredSignedGrade) {
...
Form::Form(const Form &ref)
    : type(ref.type), name(ref.name),
      requiredExecuteGrade(ref.requiredExecuteGrade),
